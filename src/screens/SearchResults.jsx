...
const SearchResults = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      try {
        const response = await fetch(`https://jewelsshop.onrender.com/api/auth/products/search?name=${encodeURIComponent(searchTerm)}`, {
          headers: {
          ...
          {results.map((item) => (
            <div key={item._id} className="col-12 col-md-6 col-lg-3 mb-4">
              <Card
                ...
                item={item}
                options={item.options} // Pass transformed options
                ImgSrc={item.img}
              />
            </div>
          ))}
